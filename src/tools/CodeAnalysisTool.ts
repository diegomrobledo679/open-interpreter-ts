...
export async function executeSyntaxCheckTool(args: { code: string; language: string }): Promise<string> {
  ...
  let fullFilePath: string; // Declare fullFilePath here
...
export async function executeExecuteLinterFormatterTool(args: { code: string; language: string; command: string; args?: string[] }): Promise<string> {
  const tempDir = os.tmpdir();
  ...
  switch (args.language.toLowerCase()) {
    ...
    case 'rust': fileExtension = 'rs'; break;
    ...
    case 'groovy': fileExtension = 'groovy'; break;
    case 'scala': fileExtension = 'scala'; break;
    case 'powershell': fileExtension = 'ps1'; break;
